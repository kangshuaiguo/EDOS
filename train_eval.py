    ...
    for epoch in range(config.num_epochs):
        ...
        for i, (trains, labels) in enumerate(train_iter):
            ...
            if total_batch % 100 == 0:
                ...
                train_acc = metrics.f1_score(true, predic, average="macro")
                dev_acc, dev_loss = evaluate(config, model, dev_iter)
                test_acc, test_loss, test_report, test_confusion, f1 = evaluate(config, model, test_iter, test=True)
                msg = 'Test Loss: {0:>5.2},  Test Acc: {1:>6.2%}, Test f1: {2:>6.2%}'
                print(msg.format(test_loss, test_acc, f1))
                if f1 > dev_best_acc:
                    dev_best_acc = f1
...
def evaluate(config, model, data_iter, test=False):
    ...
    acc = metrics.f1_score(labels_all, predict_all)
    if test:
