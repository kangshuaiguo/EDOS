...
class Model(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.fc1 = nn.Linear(config.num_filters * len(config.filter_sizes), config.inner_size)
        self.fc2 = nn.Linear(config.inner_size, config.num_classes)

        self.sigmoid = nn.Sigmoid()
        self.bn = nn.BatchNorm1d(config.num_filters)
        self.bn2 = nn.BatchNorm1d(config.inner_size)
        self.relu = nn.ReLU(inplace=True)
        self.loss_fn = focal_loss(alpha=0.25, gamma=5, num_classes=config.num_classes)
